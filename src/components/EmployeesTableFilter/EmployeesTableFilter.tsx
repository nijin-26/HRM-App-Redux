...
const EmployeesTableFilter: React.FC = () => {
  ...
  const handleSkillSelectChange = () => {
    if (
      JSON.stringify(skillFilter) === JSON.stringify(prevSkillFilter.current)
    ) {
      return;
    }
    dispatch(employeeListClear());
    ...
    if (!skillFilter.length) {
      ...
      const selectedOptionsValue = skillFilter.map((option) => option.value);
  ...
  useEffect(() => {
    if (selectSkillsData) {
      const urlSelectedSkillValues = searchParams.get("skillIds")?.split(",");
  ...
  return (
    <StyledEmployeesFilterWrap>
      <Input
        ...
        value={empNameFilter}
        onChange={(event) => {
          setEmpNameFilter(event.target.value.trimStart().toLowerCase());
        }}
